...
it("SHOULD call supabase for family-members and family correctly WHEN the user has families", async () => {
  ...
  expect(spies.supabase.from).toHaveBeenNthCalledWith(1, "family_members");
  expect(spies.supabase.from).toHaveBeenNthCalledWith(2, "families");
...
it.each([[], null])(
  ...
  async (data) => {
    ...
    expect(spies.supabase.from).not.toHaveBeenCalledWith("families");
  },
);
